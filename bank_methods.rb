#Create a class to set up a template for each account
class Account
...
#checks to make sure there is enough balance for the withdrawal
...
def main_menu
	...
	puts "Welcome to Big Bank.  Please "
	puts "make a selection below:"
	puts "-----------------------------"
...
def view_account
	...
	while @account_login < 4
		...
		if account_login(name, acct_num, password)
			@account_login = 0
			view_account_menu(name, acct_num, password)
		else
...
def view_account_menu(name, acct_num, password)
	...
	if option == 1
		view_balance(name, acct_num, password)
		view_account_menu(name, acct_num, password)	
	elsif option == 2
		make_deposit(name, acct_num, password)
		view_account_menu(name, acct_num, password)	
	elsif option == 3
		make_withdrawal(name, acct_num, password)
		view_account_menu(name, acct_num, password)	
	elsif option == 4
		main_menu
	else
...
def view_balance(name, acct_num, password)
	
	@accounts.each do |account|
		
		if account.name == name && account.account_number == acct_num && account.password == password
...
def make_deposit(name, acct_num, password)
...
def make_withdrawal(name, acct_num, password)
